...
\begin{document}
	...
	\begin{enumerate}
		...
		$3(3d+8q)=250$ \\
		$3d+8q=\dfrac{250}{3}$ \\
		$3d+8q \in \mathbb{Z}^+$ but $\dfrac{250}{3} \not\in \mathbb{Z}^+$. Addition and multiplication is closed on $\mathbb{Z}^+$ so there is no $d,\, q \in \mathbb{Z}^+$ to satisfy the constraints.
		}	
		...
		\begin{enumerate}
			...
			Two of the multiple of 5 factors $20!$ are not multiples of 10 already. \\
			...
			Eight 10s can be factored out of $20!$, therefore there are 8 trailing zeros.
		\end{enumerate}
		}
		...
		\begin{enumerate}
			...
			\textbf{Theorem A:}
			\begin{itemize}
				\item [] For all positive integers $n$, $r$, and $s$, if $rs<n$, then $r\leq\sqrt{n}$ or $s\leq\sqrt{n}$. \\
				\textbf{Proof:} \\
				Assume contrapositive. \\
				For all positive integers $n$, $r$, and $s$, if $r>\sqrt{n}$ and $s>\sqrt{n}$, then $rs \geq n$. \\
				Multiplying the inequalities $r>\sqrt{n}$ and $s>\sqrt{n}$ we get $rs>n$. \\
				$rs>n \Rightarrow rs \geq n$ therefore contrapositive is true. \\
				Contrapositive is true therefore original proposition is true.
			\end{itemize}
			$\forall n>1 \in \mathbb{Z},\, n \not\in \mathbb{P} \Rightarrow \exists p \in \mathbb{P} \text{ such that } p \leq \sqrt{n} \text{ and } p \mid n$ \\
			$\begin{aligned}
				& \exists p \in \mathbb{P} \text{ such that } p \leq \sqrt{n} \text{ and } p \mid n \\
				\equiv & \underbrace{\exists p \in \mathbb{P} \text{ such that } p \leq \sqrt{n}}_A \text{ and } \underbrace{\exists p \in \mathbb{P} \text{ such that } p \mid n}_B
			\end{aligned}$ \\
			By Theorem 4.3.4, B is true for all $n$, so we only have to prove or disprove A. \\
			$\forall n>1 \in \mathbb{Z},\, n \not\in \mathbb{P} \Rightarrow \exists p \in \mathbb{P} \text{ such that } p \leq \sqrt{n}$ \\
%			$p \mid n \therefore \exists q \in \mathbb{Z}$ such that $pq=n$ \\
			
%			By Theorem 4.3.1 $p<n$. \\
%			Let $q \in \mathbb{Z}^+$ be the other factor paired with $p$. \\
%			 $pq=n$ \\
		\end{enumerate}
		}
