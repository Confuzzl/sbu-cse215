...
%	$m+n+2\sqrt{mn}$ is a perfect square.
	}
	...
	\begin{tabular}{|l|}
		...
		\hline
	\end{tabular} \\
	The only valid, relevant case shows that both roots must be rational, therefore proposition is true.
	}
	...
	\problem{3}{
	\item [42c.]
	Without computing the value of $\left(20!\right)^2$ determine how many zeros are at the end of this number when it is written in decimal form. Justify your answer. \\
	8 zeroes.\\
	\textbf{Proof:} \\
	...
	$a^2$ has no 10s to factor out for the same reason $a$ doesn't. \\
	...
	\begin{tabular}{|l|l|l|}
		...
		\hline
	\end{tabular} \\
	In all cases, the product of consecutive integers can be written in the form $3k$ or $3k+2$ for some integer $k$, therefore proposition is true.
	}
	...
	\begin{tabular}{|l|l|}
		...
		\hline
	\end{tabular} \\
	In all cases, $E=n(n^2-1)(n+2)$ is divisible by 4, therefore proposition is true.
	}
	...
	\begin{tabular}{|l|}
		...
		$ab=c \Rightarrow \dfrac{j}{k}b=\dfrac{m}{n}$ where $j$, $k\not=0$, $m$, $n\not=0 \in \mathbb{Z}$ \\
		$b=\dfrac{mk}{nj}$ \\
		$mk,\, nj \in \mathbb{Z}$ \\
		$b$ is a ratio of two integers, therefore $b \in \mathbb{Q}$. \\
		Contradiction because $b \in \mathbb{I}$ and $b \in \mathbb{Q}$, therefore assumption is false and $ab \in \mathbb{I}$. \\
		\hline
	\end{tabular} \\
	Let $a$ be an arbitrary irrational number. \\
	Assume opposite and $\dfrac{1}{a} \in \mathbb{Q}$. \\
	...
	\rawproblem{
	Show that among any set of arbitrary (1 trillion + 1) natural numbers, one can find two numbers so that their difference is divisible by 1 trillion. \\
	\textbf{Proof:} \\
	Let $\mathbb{S} \subset \mathbb{N}$ be the set of $10^{12}+1$ arbitrary natural numbers. \\
	Let $\mathbb{T}$ be the set created by mapping every element $e \in \mathbb{S}$ under $e$ \text{mod} $10^{12}$. \\
	The maximum cardinality of $\mathbb{T}$ is $10^{12}$. By the quotient remainder theorem, remainders of division by $10^{12}$ are integers in the range $\left[0,10^{12}\right)$. The largest $\mathbb{T}$ possible contains all these remainders. \\
